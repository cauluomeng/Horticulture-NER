...
from tkinter  import * #使用Tkinter的
from tkinter.ttk import *#使用ttk的Frame, Button, Label, Style, Scrollbar等功能
...
# 定义主界面
        ...
        # recButton = Button(self, text="开启推荐功能", command=self.setInRecommendModel)
        # recButton.grid(row=2, column=self.textColumn +1)

        # noRecButton = Button(self, text="关闭推荐功能", command=self.setInNotRecommendModel)
        # noRecButton.grid(row=3, column=self.textColumn +1)

        # ubtn = Button(self, text="重定映射关系", command=self.renewPressCommand)
        # ubtn.grid(row=4, column=self.textColumn +1, pady=4)

        # ubtn = Button(self, text="新映射关系", command=self.savenewPressCommand)
        # ubtn.grid(row=5, column=self.textColumn +1, pady=4)

        exportbtn = Button(self, text="保存", command=self.generateSequenceFile)
        exportbtn.grid(row=2, column=self.textColumn + 1, pady=4)


        cbtn = Button(self, text="退出程序", command=self.quit)
        ...
        # self.RecommendModelName = Label(self, text="推荐功能: ", foreground="Blue", font=(self.textFontStyle, 14, "bold"))
        ...
        for idx in range(0, len(self.allKey)):
            ...
            self.text.bind(press_key, self.textReturnEnter)   #控件事件与控件绑定
        ...
        self.text.bind('<Control-Key-z>', self.backToHistory)   # 撤回
    ...
    def singleLeftClick(self, event):
        ...
        row_column = cursor_index.split('.')     #以"."为分隔符，每一句算一行
    ...
    def rightClick(self, event):
        ...
        try:
            ...
            content = self.text.get('1.0',"end-1c").encode('utf-8')  # 获取从开始到结尾所有内容
...
# open按钮功能   打开文件
    def onOpen(self):
        ...
        # dlg = tkinter.filedialog.askopenfilename(**options)
        fl = dlg.show()
        if fl != '':
...
#执行命令
    def executeCursorCommand(self,command):
        ...
        try:
            firstSelection_index = self.text.index(SEL_FIRST)  #选中部分起始
            cursor_index = self.text.index(SEL_LAST)  #选中部分最后
...
#执行输入框Command命令
    ...
    #     if len(command) == 0:
    #         currentCursor = self.text.index(INSERT)  #定位光标
    ...
    #     else:
    #         command_list = decompositCommand(command)   #获取命令
    #         for idx in range(0, len(command_list)):
    #             command = command_list[idx]
    #             if len(command) == 2:    #命令是2位数
    #                 select_num = int(command[0])  #位数
    #                 command = command[1]   #标注类型命令
    #                 content = self.getText()
    #                 cursor_index = self.text.index(INSERT)
    ...
    def deleteTextInput(self,event):
        ...
        #followHalf_content = str(followHalf_content)
        #aboveHalf_content = str(aboveHalf_content)
        if len(get_input) > 0:
...
#添加推荐的内容
    def addRecommendContent(self, train_data, decode_data, recommendMode):
        ...
        if not recommendMode:
            ...
            content = maximum_matching(train_data, decode_data,'plant.txt')
            generateDictFile('plant.txt', train_data)
        DictFilerRank('plant.txt', train_data)
        return content
...
#加载打开新文件
    ...
    ## update shortcut map  REmap按钮 更新快捷键
    def renewPressCommand(self):
        ...
        self.setMapShow()
        tkinter.messagebox.showinfo("Remap Notification", u"快捷方式已更新！\n\n配置文件保存在：" + self.configFile)

    ## save as new shortcut map   NewMap按钮  保存新的快捷键组合
    ...
    def setMapShow(self):
        ...
        mapLabel = Label(self, text ="词语类型映射快捷键", foreground="blue", font=(self.textFontStyle, 14, "bold"))
    ...
    #     # self.configListLabel = Label(self, text="映射模板", foreground="blue", font=(self.textFontStyle, 14, "bold"))
...
#生成序列标注文件
    def generateSequenceFile(self):
        ...
        tkinter.messagebox.showinfo(u"导出信息", showMessage)
...
def decompositCommand(command_string):
    ...
    for idx in range(0, len(command_string)):
        if command_string[idx].isdigit():
            num_select += command_string[idx]    #存数字
        else:
            each_command.append(num_select)
            each_command.append(command_string[idx])
            command_list.append(each_command)   #存整个命令
            each_command = []
            num_select =''
    # print command_list
    return command_list
...
def main():
    ...
    root = Tk()
    root.geometry("1450x700+200+200")
    app = Example(root)
    app.setFont(17)
    root.mainloop()
